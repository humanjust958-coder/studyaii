...
export function TimetableGen() {
  const { profile, addXP } = useUser();
  const [topic, setTopic] = useStickyState('', 'studygenie_timetable_topic');
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useStickyState<string | null>(null, 'studygenie_timetable_result');

  const customColor = localStorage.getItem('setting_timetable_color');
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      const moduleContext = `You are a Smart Study Planner. Given the user's exam constraints or weekly goals string, produce a structured 7-day or focused study table/timeline aligning with their preferences: ${profile.studyTime} time preference and ${profile.dailyHours} hours/day. Formatting must be clear markdown.`;
      const systemPrompt = generateSystemPrompt(profile, moduleContext);
      const res = await askGemini(profile.apiKey, `Plan for: ${topic}`, systemPrompt);
      setResult(res);
      addXP(10);
    } catch (e) {
      ...
      <div className="glass-card p-6 md:p-8">
        <div className="flex justify-between items-start mb-6">
          <h2 className={`text-3xl font-display font-bold flex items-center gap-3 ${headingColorClass}`}>
            <Calendar className={`w-8 h-8 ${iconClass}`} />
            AI Timetable Generator
            <TutorialButton 
              title="Timetable Generator" 
              instructions={[
                "Describe your upcoming goal (e.g., 'Learn Optics in 5 days').",
                "The AI will use your profile settings (hours per day, study time) to build a custom plan.",
                "Review the plan and share or copy it."
              ]} 
              resources={[
                { label: "Time Blocking Strategy", url: "https://todoist.com/productivity-methods/time-blocking" }
              ]}
            />
          </h2>
        </div>
        ...
        <div className="flex flex-col mb-8">
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <input
              type="text"
              placeholder="E.g., Board Exams in 2 weeks, Need to cover Science..."
            ...
            >
              <Send className="w-5 h-5"/> Plan It
            </button>
          </div>
        ...
        <AnimatePresence mode="wait">
          {loading && (
            <motion.div key="loading" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              <AILoadingIndicator text="Structuring your time..." />
            </motion.div>
          )}
          {!loading && result && (
             <motion.div 
